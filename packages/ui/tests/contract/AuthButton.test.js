...
describe('AuthButton Contract', () => {
    ...
    describe('Component Interface', () => {
        it('should accept required props with correct types', () => {
            expect(() => {
                render(_jsx(AuthButton, { mode: "signin", onSuccess: () => { }, onError: () => { } }));
            }).not.toThrow();
        });
        it('should accept optional props with correct types', () => {
            expect(() => {
                render(_jsx(AuthButton, { mode: "signup", variant: "outline", size: "lg", text: "Custom Sign Up", loadingText: "Signing up...", disabled: false, className: "custom-auth-button", onSuccess: () => { }, onError: () => { }, onLoadingStateChange: () => { } }));
            }).not.toThrow();
        });
        it('should handle mode switching', () => {
            const { rerender } = render(_jsx(AuthButton, { mode: "signin", onSuccess: () => { }, onError: () => { } }));
            expect(screen.getByRole('button', { name: /sign in/i })).toBeInTheDocument();
            rerender(_jsx(AuthButton, { mode: "signup", onSuccess: () => { }, onError: () => { } }));
    ...
    describe('Rendering Contract', () => {
        ...
        it('should display correct text for signup mode', () => {
            render(_jsx(AuthButton, { mode: "signup", onSuccess: () => { }, onError: () => { } }));
            expect(screen.getByRole('button', { name: /sign up/i })).toBeInTheDocument();
        });
        it('should display custom text when provided', () => {
            render(_jsx(AuthButton, { mode: "signin", text: "Custom Login", onSuccess: () => { }, onError: () => { } }));
        ...
        it('should show loading state with spinner', () => {
            ...
            render(_jsx(AuthButton, { mode: "signin", loadingText: "Signing in...", onSuccess: () => { }, onError: () => { } }));
            expect(screen.getByRole('button', { name: /signing in/i })).toBeInTheDocument();
            expect(screen.getByLabelText(/loading/i)).toBeInTheDocument();
        });
        it('should apply different variants correctly', () => {
            const { rerender } = render(_jsx(AuthButton, { mode: "signin", variant: "default", onSuccess: () => { }, onError: () => { } }));
            const button = screen.getByRole('button');
            expect(button).toHaveClass('bg-primary'); // shadcn/ui default button style
            rerender(_jsx(AuthButton, { mode: "signin", variant: "outline", onSuccess: () => { }, onError: () => { } }));
            expect(button).toHaveClass('border'); // shadcn/ui outline button style
        });
        it('should apply different sizes correctly', () => {
            const { rerender } = render(_jsx(AuthButton, { mode: "signin", size: "default", onSuccess: () => { }, onError: () => { } }));
            const button = screen.getByRole('button');
            expect(button).toHaveClass('h-10'); // shadcn/ui default button height
            rerender(_jsx(AuthButton, { mode: "signin", size: "lg", onSuccess: () => { }, onError: () => { } }));
    ...
    describe('Interaction Contract', () => {
        ...
        it('should call onSuccess when authentication succeeds', async () => {
            ...
            render(_jsx(AuthButton, { mode: "signin", onSuccess: onSuccess, onError: () => { } }));
            await waitFor(() => {
        ...
        it('should call onError when authentication fails', async () => {
            ...
            render(_jsx(AuthButton, { mode: "signin", onSuccess: () => { }, onError: onError }));
            const button = screen.getByRole('button', { name: /sign in/i });
            fireEvent.click(button);
            await waitFor(() => {
        ...
        it('should call onLoadingStateChange during auth flow', async () => {
            ...
            render(_jsx(AuthButton, { mode: "signin", onSuccess: () => { }, onError: () => { }, onLoadingStateChange: onLoadingStateChange }));
            const button = screen.getByRole('button', { name: /sign in/i });
            fireEvent.click(button);
            expect(onLoadingStateChange).toHaveBeenCalledWith(true);
        });
        it('should not trigger auth when disabled', () => {
            render(_jsx(AuthButton, { mode: "signin", disabled: true, onSuccess: () => { }, onError: () => { } }));
            const button = screen.getByRole('button', { name: /sign in/i });
    ...
    describe('Accessibility Contract', () => {
        it('should have no accessibility violations', async () => {
            const { container } = render(_jsx(AuthButton, { mode: "signin", onSuccess: () => { }, onError: () => { } }));
    ...
    describe('Widget Integration Contract', () => {
        it('should integrate with widget system', () => {
            ...
            expect(() => {
                render(_jsx(AuthButton, { mode: "signin", onSuccess: () => { }, onError: () => { }, ...widgetProps }));
            }).not.toThrow();
        });
        it('should support webhook data integration', () => {
            ...
            expect(() => {
                render(_jsx(AuthButton, { mode: "signin", webhookData: webhookData, onSuccess: () => { }, onError: () => { } }));
            }).not.toThrow();
        });
    });
